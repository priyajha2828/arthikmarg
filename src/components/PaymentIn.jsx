...
function EmptyState({ title, description, buttonText, onClick }) {
  return (
    ...
    >
      ...
      <button
        onClick={onClick}
        className="px-6 py-3 rounded-lg text-white flex gap-2"
...
export default function PaymentIn({ sidebarOpen = true }) {
    ...
    <>
      ...
      >
        {payments.length === 0 ? (
          <EmptyState
            ...
            description="Start recording incoming payments"
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/40 flex justify-center pt-10 z-50">
          <div
            className="w-[600px] rounded-md overflow-hidden"
            style={{
              background: "var(--surface-100)",
              color: "var(--text-default)",
              maxHeight: "80vh",
